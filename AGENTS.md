...
- Verified: 254 XCTest (the snapshot test skips unless `DV_SNAPSHOT_DIR` is set); real mouse clicks select every pane and open a search result; 20 offscreen renders reviewed in light and dark. Not yet installed or tried by hand.
- Details: `docs/plans/2026-09-22-settings-window-design.md` and `-plan.md`. Pending: search result ranking (owner), Developer ID build + hands-on check, #31 confirmation on macOS 15.
