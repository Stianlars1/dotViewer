...
### dotViewer 1.5.3 release
- Outcome: Merged PR #26, published notarized `v1.5.3` / build 10 to GitHub, updated Homebrew cask, and verified the live download route. Tag points to merge commit `03220c490492b306751bf91f17851c5e3eda8f53`.
- Packaging: Fresh Xcode archive attempts stalled in compiler-probe output handling. Promoted the previously successful, tested Developer ID 1.5.3 (10) candidate instead; verified no application-source differences between tested commit `7a7b02b` and the merge commit. Included both new grammars' MIT notices in the installer. No product code was altered for this workaround.
- Verified: Apple accepted app and DMG notarization; both tickets stapled; Gatekeeper accepts both as Notarized Developer ID. Mounted DMG has app, Applications link and third-party notices. Installed app payload matches the release candidate (235 files/symlinks, excluding separately validated stapler receipt).
- Published: https://github.com/Stianlars1/dotViewer/releases/tag/v1.5.3 ; Homebrew tap commit `fc8122c`. Public website download resolves to the 1.5.3 DMG, whose downloaded SHA-256 matches GitHub and the cask: `b76733f665bddfbaec68a27753534bf39ef61dc2dc735de4321dac9f6e17f5b0`.
- Evidence: `/tmp/dotviewer-release-1.5.3/` contains notarization logs, installer, checksum and fetched release metadata. GPX post-release Quick Look smoke passed; prior 204 XCTest and 10 Python checks cover the unchanged app source. Native thumbnail routing limitation remains as recorded above.

## 2026-09-09

### Status-page hang and installer-layout recovery
- Outcome: Replaced wait-before-read process pipes with bounded file-backed status commands, added five-second deadlines and nonzero-exit handling, narrowed the registration query, and made loading/error/conflict states explicit. Unknown/superseded registrations and failed conflict mutations no longer report green success.
- Packaging: Restored mandatory DropDMG `dotviewer` layout; removed automatic plain-DMG fallback and stale-output discovery. Added explicit version/build-checked export reuse for interrupted release workflows, documented Apple compiler/signing recovery, and moved third-party notices inside the app.
- Files: `Utilities/StatusProcessRunner.swift`, `Utilities/ExtensionStatusChecker.swift`, `Utilities/ExtensionConflictScanner.swift`, `App/StatusView.swift`, `scripts/package-dmg.sh`, release/publish scripts, tests and `docs/releases/WORKFLOW.md`.
- Verified: Live 1.5.3 pluginkit process samples blocked in stdout writes; old implementation also present in v1.5.0. Isolated old runner deadlocked on 256 KiB output. New runner passed large stdout/stderr, concurrency, timeout, launch-failure and nonzero-exit checks. 219 XCTest tests and 13 Python tests passed. Native GUI reached green and refresh checks completed; an initial post-install timeout displayed a retryable error and a refresh recovered. DropDMG purple layout, arrow and icon placement verified in Finder against the user's 1.5.0 reference.
- Build environment: Xcode 26.6 compiler probes and export codesign children independently stalled in pipe writes. Native Apple compiler forwarding and explicit Developer ID signing were used with strict bundle verification; both arm64 and x86_64 were built. Recovery is documented rather than silently substituting an unstyled installer.

### 1.5.4 hotfix released with restored DropDMG layout
- Outcome: Merged PR #27 and published 1.5.4 (11) through `publish.sh --reuse-exported-app --build-number=11`, using the newly built and strictly verified Developer ID export. App and DMG were notarized/stapled successfully. Homebrew tap updated in `2f4f474`.
...
### Settings window redesign (2026-09-22, shipped in 1.5.8)
- Outcome: Settings moved from a main-window sidebar page with custom tab bars into a `Settings` scene (dotViewer → Settings…, ⌘,): a `NavigationSplitView` with a native sidebar list of seven panes and grouped `Form` panes (name and description leading, control trailing, switches). Sidebar search over `SettingsCatalog` opens the pane and highlights the row. Markdown settings moved in; the main sidebar keeps Status and File Types plus a ⌘, footer link. `SettingsView`, `MarkdownSettingsView`, `SettingsTabPage`, `QuickLookFindKeySettings`, `PreviewPanelSettings` and `SettingsTabBarTests` removed.
- Files: `dotViewer/App/Settings/` (window, pane enum, catalog, model, rows, footer link, `Panes/`), `App/dotViewerApp.swift`, `App/ContentView.swift`, `project.yml`, `dotViewerTests/Settings{Catalog,Sidebar,Snapshot}Tests.swift`.
- Verified: 256 XCTest (the snapshot test skips unless `DV_SNAPSHOT_DIR` is set); real mouse clicks select every pane and open a search result; 20 offscreen renders reviewed in light and dark. A local Developer ID 1.5.8 build, installed in place over the public 1.5.7 (same designated requirement, Accessibility and Finder access intact), was checked through `scripts/dotviewer-ax.swift`: real ⌘,, the menu item and the footer open Settings; every pane shows stored values; typed search ranks titles first and opens results; a switch flipped in Settings reaches the Quick Look extension's log.
- Fixed from the hands-on check: sidebar at 140 pt and window at 900 × 532 (`.toolbar(removing:)` must precede the column width; the Settings scene needs `.defaultSize`), pinned by `SettingsWindowLayoutTests`. Found then: "Interface text size" never worked on macOS (KI-020, fixed below).
- Details: `docs/plans/2026-09-22-settings-window-design.md` and `-plan.md`. Pending: #31 confirmation on macOS 15.
- Lessons: offscreen, `NavigationSplitView` columns do not draw into `cacheDisplay`, so the snapshot test composes sidebar and pane in a plain `NSHostingView`. Building the app scheme (Debug) registers its extensions and displaces the `/Applications` Quick Look registration at the same version; compile App code through the test scheme instead.

### 1.5.8 release and stats phase 1 deploy (2026-09-23)
- Outcome: `feat/usage-stats-and-updates` fast-forwarded onto `main`, `feat/settings-window` rebased and fast-forwarded, `chore(release): prepare 1.5.8`, pushed; Vercel deployed the site from the push. Released 1.5.8 (15) with `publish.sh` (GitHub release, Homebrew cask). Before the push: `site/db/sql/001` applied to production, backfill `--apply` (no scrub), `CRON_SECRET` set.
- Verified: 264 XCTest, 26 site tests, typecheck, `next build`; checksum = GitHub asset = local DMG = Homebrew cask; DMG and app notarized, stapled, Gatekeeper-accepted, universal; site serves 1.5.8; live site sets no cookies, `/stats` fails closed (503) until the owner adds `STATS_USER`/`STATS_PASSWORD`, cron `ok` with the secret and registered; public build installed in place with Accessibility intact.
- Details: `docs/releases/1.5.8-verification.md`, §10 of `docs/plans/2026-09-22-usage-stats-telemetry-updates.md`.

### Interface text size and main window frame (2026-09-23, shipped in 1.5.8)
- Outcome: KI-020 fixed with text-only scaling: presets are body sizes 11–21 pt around 13; the window root gets the scaled body font and an `appTextScale` environment value; styled text uses `appFont(_:)`; List rows and Form section headers/footers use `appFontWhenScaled(_:)` so the system's own styling stays at Default; `SettingsSection` gives titled sections a header that stays bold. Fixed widths that hold text (status badge, stat cards, numbered circles, sheets, sidebar columns, slider value column) scale; the sidebar footer drops its ⌘, keycap before "Settings" could wrap; File Types category labels get a scaled icon column. The main `WindowGroup` got `id: "main"`, and `StaleWindowFrames` removes the per-build frame keys older builds left (61 on this Mac).
- Files: `App/AppUIFontSizing.swift`, every App view with a font, `App/Settings/SettingsRows.swift` (`SettingsSection`), `App/StaleWindowFrames.swift`, `App/dotViewerApp.swift`, `Shared/SharedSettings.swift` (`medium` dropped), `project.yml` (main-window views now compile in the test target), `dotViewerTests/{AppUIFontSizing,StaleWindowFrames}Tests.swift`, `SettingsSnapshotTests.testRenderTextSizes`.
- Verified: 264 XCTest; at Default every Settings render is pixel-identical to the pre-change build except the edited Interface text size row; renders at Extra Small/Large/XXX Large reviewed. Installed as a local Developer ID 1.5.8 (15): at Small the main window's sidebar rows, titles and captions shrink; switching to XXX Large in Settings resized both windows live (sidebar rows 16 → 25 pt tall, headers stay bold, switches unchanged); stale frame keys gone; Quick Look smoke test passes.
- Lessons: macOS ignores `dynamicTypeSize`. A grouped Form's section headers take the window's font (losing bold) and its switches turn regular-size on any `controlSize` environment write, even a no-op `transformEnvironment`. Sidebar List rows ignore the window's font but take one set on the row. `xcodebuild -exportArchive` can fail with "A timestamp was expected but was not found" when Apple's timestamp service hiccups; rerun just the export on the existing archive.

### Analytics scrub and consent banner (2026-09-23, live)
- Outcome: on the owner's request the live analytics rows' identifying values were scrubbed (row counts unchanged; §10 of the stats plan). The owner then asked for a self-made cookie banner to allow tracking; designed together (spec and plan in `docs/plans/2026-09-23-consent-banner-*.md`) and built on `feat/consent-banner`: a consent card (Reject and Accept equal, Choose…, GPC counts as Reject, hidden on `/stats`), `POST /api/consent` (same-origin only, 512-byte cap, 204 + cookies, record written in `after()`), a daily visitor code for everyone (salted SHA-256, the salt deleted after its UTC day), `dv_visitor` and Google Analytics only with consent (gtag.js never requested before; withdrawing deletes `_ga*`), consent records, retention in the snapshot cron, a Cookies section on `/privacy`, and a Visitors section on `/stats`. Schema change `site/db/sql/002-consent-and-day-visitors.sql` is additive.
- Files: `site/lib/consent/`, `site/lib/analytics/{day-visitor,context,server,client}.ts`, `site/app/api/consent/route.ts`, `site/components/{consent-banner,cookie-settings-button,site-analytics}.tsx`, `site/lib/stats/{visitors,retention,queries,report}.ts`, `site/app/{stats,privacy}/`, `site/app/api/cron/snapshots/route.ts`, `site/lib/db/schema.ts`, READMEs.
- Verified: 50 site tests, typecheck, `next build`; local end to end against a throwaway Postgres; the owner approved the branch preview. Rollout: `002` applied to production (805 page views and 225 downloads unchanged), `NEXT_PUBLIC_GA_MEASUREMENT_ID` added for Production before the build, `main` fast-forwarded to `23a96da`, then verified live: no cookie or Google request before a choice, Accept sets the cookies and loads GA, withdrawing deletes GA's `.dotviewer.app` cookies, consent records and day codes land in the database (spec, both Verification sections). Consent records 1 and 2 were that test (deleted afterwards).
- Rolling back: Vercel's Instant Rollback only; redeploying a pre-banner commit would bake the GA ID into code that loads GA for everyone.
- Lessons: the built-in browser's user agent contains `Claude/`, so the site logs its visits as bots. Vercel previews are behind Vercel login (302); `vercel curl` may create a protection-bypass secret on the project, so the owner opens the preview instead. `next dev` rewrites `site/next-env.d.ts` to `.next/dev/types`; restore it before committing. `drizzle-kit push` always "recreates" the composite primary keys of the snapshot tables, even right after creating them: a quirk, not drift.

### `/stats` review follow-ups (2026-09-23)
- Outcome: from the owner's screenshot of `/stats`. Old rows had no `referrer_host` (added in `001`; the backfill never derived it), so Referrers was empty, yet they still held the full linking address, which `/privacy` says is not kept; page addresses were stored with their query strings. `lib/analytics/payload.ts` now drops everything after `?` or `#` from page and download addresses (UTM tags arrive in their own fields); `db/sql/003` derived the site names (797 page views, 96 downloads), deleted the full addresses and trimmed 53 page addresses; `/privacy` updated. The charts share one `BarChart`: a fixed-height SVG stretched sideways (`preserveAspectRatio="none"`) with the dates as HTML, so they no longer grow with the window. The two test consent records were deleted.
- Verified: 51 site tests, typecheck, build; locally `003` matched the live TypeScript rules on 17 edge-case addresses and changed nothing on a second run; on production a rolled-back dry run, then the real run (no full addresses or query strings left, row counts unchanged); a live visit with `?fbclid=` was stored without it; charts checked at 1500 and 375 px.
- Lessons: a backfill that adds a derived column has to derive it for the old rows too, or reports silently read nothing; check what `/privacy` promises against every stored column, not only the identifiers.
