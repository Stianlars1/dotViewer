...
- **2026-02-12**: Fixed markdown links (KI-012): clicking links now copies URL to clipboard with toast confirmation ("Link copied" / "Path copied"), tooltip on hover. Added font size sync: new `syncFontSizes` setting (default ON) makes code and markdown rendered view share one font size. Toggle in Settings > Appearance, disabled slider with note in Markdown settings when synced.
