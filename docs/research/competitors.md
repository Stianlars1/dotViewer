...
### Status Note (dotViewer v2.5)
This document describes the v1-era performance bottleneck (JavaScript-based highlighting) and the competitor patterns that motivated the rewrite.

As of dotViewer v2.5, the architecture has moved to:
- an **XPC-based highlighting pipeline**
- a **native Tree-sitter highlighter**

So if Quick Look “launches but does nothing” or falls back to system preview, treat it as a **discovery/registration problem first** (missing `NSExtension` keys, wrong `QLSupportedContentTypes`, missing sandbox/App Group entitlements), not a performance problem.
Also confirm the preview extension is truly **data-based**: the principal class should subclass `QLPreviewProvider` and implement `providePreview(for:completionHandler:)` (the method Quick Look invokes).
Finally, confirm **UTI routing**: for many code/script UTIs on macOS 15+, Quick Look appears to require an **exact** UTI identifier
in `QLSupportedContentTypes` (e.g. `public.python-script`, `public.swift-source`, `com.netscape.javascript-source`). Listing only
`public.source-code` / `public.text` often won’t cause the extension to be selected.
