...
- Summary: Full app + Quick Look extension with XPC rendering using the Highlight C++ library.
- Primary file types: Source code, plain files without extension
- Plugin type: Quick Look App Extension (.appex) + XPC service
- License: GPLv3 (LICENSE.txt)
- Feature tags: syntax-highlighting, highlight, xpc, app-extension, rtf, html
...
|   |-- PlainSettingsView.swift
|   |-- PlainSettingsView.xib
|   |-- PreviewView.swift
|   |-- PreviewView.xib
|   |-- SaveAsFormatView.swift
|   |-- SaveAsFormatView.xib
|   |-- SCSHApp.entitlements
|   |-- SCSHTheme+ext.swift
|   |-- SCSHWrapper.swift
|   |-- Settings+App.swift
...
|   |-- xcshareddata
|   |   `-- xcschemes
|   |       |-- extra_resources.xcscheme
|   |       |-- QLSourceSyntaxGUITest.xcscheme
|   |       |-- relaunch.xcscheme
|   |       |-- settings_chk.xcscheme
|   |       |-- ShortcutCommand.xcscheme
|   |       |-- Syntax Highlight (no XPC debug for Sparkle test).xcscheme
|   |       |-- Syntax Highlight Quicklook Extension.xcscheme
|   |       |-- Syntax Highlight XPC Render.xcscheme
|   |       |-- Syntax Highlight XPC Service.xcscheme
|   |       |-- Syntax Highlight.xcscheme
|   |       |-- syntax_highlight_cli.xcscheme
|   |       `-- yaml checker.xcscheme
|   `-- project.pbxproj
...
- `QLExtension/PreviewViewController.swift`: QL extension entry; dispatches to rendering service.
- `SyntaxHighlightRenderXPC/` and `XPCService/`: non-sandboxed XPC services that run Highlight.
- `highlight-wrapper/`: C++ bridge layer for Highlight library.
- `Application/Settings*` and `Settings+Service.swift`: settings UI and shared configuration.
- `Resources/` and `share/`: themes, language definitions, and bundled assets.

## Non-Relevant Paths (scanned)
- Assets, screenshots, and help bundles.

## Architecture Notes
- Quick Look extension communicates with an XPC service to do heavy highlighting.
- Supports HTML and RTF render paths depending on macOS version.

## Performance Tactics
- XPC isolation prevents UI blocking; Highlight runs natively.
- Supports max data size limits and cancels work early.

## Build / Setup Notes
- Requires git submodules; Xcode workspace builds app, extension, and XPC.

## Reuse Notes
- Best reference for modern extension + XPC architecture and sandbox workarounds.
