...
dotViewer v2.5 has already adopted the two biggest competitor takeaways:
- **XPC isolation** for heavy work (highlighting)
- **Native parsing/highlighting** (Tree-sitter) instead of JavaScript

Current “it does nothing / system fallback” failures are more likely to be **extension registration** issues (missing `NSExtension` keys) or **entitlements/sandbox** issues (extensions not being discovered/instantiated), not pure performance.
For modern data-based previews, also verify the preview principal class is a `QLPreviewProvider` subclass and implements `providePreview(for:completionHandler:)` (the method Quick Look calls).
Also verify **routing**: on macOS 15+, many script/source UTIs require an **exact match** in
`NSExtensionAttributes -> QLSupportedContentTypes` (e.g. `public.python-script`, `com.netscape.javascript-source`,
`public.swift-source`). Relying on conformance to `public.source-code` / `public.text` is often not enough.
